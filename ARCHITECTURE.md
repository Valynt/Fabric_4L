...
Value Fabric uses a six-layer core microservices architecture with strict tenant isolation via PostgreSQL RLS, plus a Layer 2.5 signal-refinement stage and a Layer 7 billing service.
...
| 2.5 | layer2-5-signal-refinery | 8007 | Signal refinement, enrichment, and normalization between extraction and the knowledge graph |
...
| 7 | layer7-billing | 8008 | Subscription, usage, billing, and entitlements |
...
- **Runtime packages** — `value_fabric/layer*/` are shim packages that delegate to canonical service implementations in `services/layer*/src/`. Layer 2.5 (`value_fabric/layer2_5/`) and Layer 7 (`value_fabric/layer7/`) follow the same pattern.
...
| Concern | Path |
| --- | --- |
| Runtime Python packages | `value_fabric/layer{1–7}/`, `value_fabric/shared/` |
