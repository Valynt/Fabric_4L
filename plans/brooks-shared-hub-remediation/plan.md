...
- `shared.tracing`, `shared.projections`, `shared.billing_schemas`, `shared.testing`, `shared.tests` are the confirmed zero-runtime-consumer removals; `shared.llm_safety` (4 runtime + 1 test use) is kept but placed under the consumer policy.
