...
- [x] Create `tests/contract/compat_surface/harness.py` (+ `__init__.py`) hoisting `get_route_prefix`, `_collect_paths`, `_collect_routes`, `get_middleware_names`, and a generic `app_with_noop_lifespan`.
- [x] Rewrite L4/L5/L6 `test_compat_app_surface_contract.py` as thin per-layer files importing the harness (kept only layer-specific assertions; L4 still wires its own `build_lifespan` monkeypatch path via the harness helper). Note: the repo-root `tests` package is shadowed by each layer's own `tests` package (their `__init__.py`), so the thin files load the harness from its absolute path via `importlib` rather than `from tests.contract...` (L6 lacks `__init__.py` and would have worked either way — used the same mechanism for consistency).
- [x] Merge `test_benchmark_route_matrix_and_contracts.py` into `test_benchmark_route_matrix.py` (all 8 tests preserved: 3 lineage/happy-hostile/OpenAPI + 5 throughput/lineage/stats/403/OpenAPI-shape); rename the two module's autouse fixtures to distinct non-autouse names (`setup_repo` / `setup_repo_and_auth`) so they can coexist in one file; delete the `_and_contracts.py` file.
- [x] Validate: L4 compat (3 passed), L5 compat (4 passed), L6 compat + merged route-matrix (13 passed); full L4 tree (3 pre-existing failures — `docx` dependency missing + two enrichment enum-count drift tests; confirmed identical on committed Step-1 base, unrelated to this step), full L5 tree (516 passed, 14 skipped), full L6 tree (191 passed, 2 skipped); `pytest tests/contract` static subset (484 passed, 0 failures — unchanged from Step 1). Route-matrix coverage unchanged (merged file holds 8 tests = prior 3 + 5).
**Files:** new `tests/contract/compat_surface/harness.py` (+ `__init__.py`) (hoists `get_route_prefix`, `collect_paths`, `collect_routes`, `get_middleware_names`, `app_with_noop_lifespan`), `services/layer4-agents/tests/test_compat_app_surface_contract.py`, `services/layer5-ground-truth/tests/test_compat_app_surface_contract.py`, `services/layer6-benchmarks/tests/test_compat_app_surface_contract.py` (thin per-layer files importing the harness), `services/layer6-benchmarks/tests/test_benchmark_route_matrix.py` (merged), `services/layer6-benchmarks/tests/test_benchmark_route_matrix_and_contracts.py` (deleted).
...
- [x] Extract ~26 `_x_to_db`/`_x_from_db` serializers from `audit_orchestrator/persistence.py` (48.5KB, 1356 lines) into new `serialization.py` (795 lines); `persistence.py` re-exports them (ruff explicit `X as X` idiom) with `__all__` unchanged (6 DB classes + `PersistenceManager` + `update_knowledge_graph`). VALIDATED: ruff clean; L4 targeted pytest (persistence/graph/api/models) 181 passed in 75.38s; 3 pre-existing failures excluded (docx-dep + 2 enrichment enum-count drift, identical on committed base).
- [ ] Split `layer1_ingestion/shared/tasks.py` (2764 lines) into `tasks/` package (`__init__.py` bootstrap + `crawl/extraction/post_processing/validation/storage/notification/dlq/cleanup.py`); Celery name freezing via explicit `name=`; retarget test patch strings per governing rule (retarget iff called function's defining module ≠ `__init__`); gate = full L1 tree matches baseline 1099 passed / 276 skipped.
- [ ] Extract pure helpers from `engine/executor.py` (1641 lines) into `execution_helpers.py`; `OrchestrationController` + `WorkflowExecutor` alias intact.
**Files:**
