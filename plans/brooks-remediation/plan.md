...
**Closed as: DELETE shims, no compatibility alias (amendment).** The re-export shims served as the transition mechanism only. Because the package is pre-production and nothing outside `services/layer4-agents` depends on `layer4_agents.integrations` (verified: zero external importers, no publish workflow distributes layer4 as an SDK, no dead-code allowlist entries, no CI baseline/test references), the shims are deleted outright. The acceptance criterion supersedes "preserve for backward compatibility": no compatibility code exists solely to preserve an interface that has never been released or consumed externally.
