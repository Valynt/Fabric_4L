...
def _mark_rejected_from_dict(cand, candidates_dir, reviewer, reason, **extra_stamp):
    """Move an already-loaded candidate dict to candidates/rejected/.

    This avoids the N+1 re-read that would happen if ``mark_rejected`` loaded
    the candidate from disk again after the caller had just read it.
    """
    ...
    try:
        ...
        # File may have been moved by a concurrent process - this is acceptable
        # since we've already written the rejected copy
        pass
    except OSError as exc:
        # Log but don't fail - the rejected copy is the source of truth
        import warnings
        warnings.warn(
            f"Failed to remove staged candidate {src}: {exc}",
            RuntimeWarning,
            stacklevel=2,
        )
    _refresh_queue(candidates_dir)
...
def mark_rejected(candidate_id, reviewer, reason, candidates_dir, **extra_stamp):
    ...
    if not os.path.exists(src):
        raise FileNotFoundError(f"candidate not found: {candidate_id}")
    cand = load_candidate(src)
    return _mark_rejected_from_dict(
        cand, candidates_dir, reviewer, reason, **extra_stamp
    )
