...
Evidence authority rule: ship/no-ship decisions are authoritative only when reflected in `docs/launch/launch-blocker-register.md` and `docs/launch/environment-dependent-evidence-matrix.md`. `reports/` artifacts are supporting diagnostics only unless explicitly linked to a passing gate run with timestamp + commit SHA metadata. The canonical repository-owned evidence-pack generator is `scripts/ci/generate_release_evidence_packet.py`; do not duplicate release-readiness scoring logic in other scripts. The canonical packet outputs are `artifacts/release/evidence-packet/release-evidence-packet.json` and `artifacts/release/evidence-packet/release-evidence-summary.md`. For release-candidate review, run `pnpm evidence:bundle` to assemble the packet, launch scorecard, contract reports, migration status, CI workflow registry, observability/K8s inventories, and existing heavy CI evidence into `artifacts/evidence/value-fabric-evidence-<sha>.tar.gz`. Review `manifest.json.evidence_gaps` before treating the bundle as complete production release evidence.
