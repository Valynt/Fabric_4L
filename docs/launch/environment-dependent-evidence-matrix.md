...
|---|---|---|---|---|---|
| Production-like E2E launch rehearsal | REQUIRES_ENVIRONMENT | Test owner | Browser-executed rehearsal against the release candidate with real authentication, backing stores, persisted state, logs, and screenshots or transcript. | Critical launch journey completes without manual database intervention; failures are classified P0/P1/P2 in the blocker register. **Minimum evidence:** retained JUnit XML, trace files, screenshots/video for all 7 P0 journeys, release-candidate SHA, and service logs. | Local surrogate evidence in `signoff-evidence/p0-journeys-20260613.json` shows the candidate is re-testable (auth crash fixed, route drift remains). Staging evidence still required. |
| Enterprise SSO/OIDC provider validation | DEFERRED_TO_PAID_GA | Identity owner | Provider metadata, redirect URI validation, login/logout proof, failed-login behavior, role/group mapping, and redacted audit event. | Authentication fails closed, maps expected tenant roles, and produces auditable events without exposing secrets. | Scoped out of Core GA per `docs/launch/sso-core-ga-scope-decision.md`. Local Keycloak surrogate evidence in `signoff-evidence/p0-sso-20260613.json` validates OIDC wiring and claim shape. Enterprise IdP evidence required before paid/enterprise GA. |
...
| Rollback and restore drill | REQUIRES_ENVIRONMENT | SRE owner | Rollback command transcript, restore proof, data-integrity check, timing result, and approval record. | Rollback and restore meet the release recovery target and leave no unreconciled data changes. **Minimum evidence:** redacted transcript, before/after data-integrity check, actual recovery time versus target, and SRE owner approval. | Local drill evidence in `signoff-evidence/p0-rollback-20260613.json` documents the viable rollback doctrine. Production-like rehearsal still required. |
