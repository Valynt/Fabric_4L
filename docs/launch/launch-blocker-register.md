...
| RG-2026-05-18-02 | `make verify` canonical gate | Current local evidence: `make lint-layer4` passes with `All checks passed!`; full `make verify` has not yet been rerun to identify or clear the next blocker. | Layer 4 owner / Release owner | OPEN | Blocks launch until `make verify` is green. |
