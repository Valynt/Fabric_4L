...
| **P0-002** | Rollback and restore drill | SRE owner | **Medium** | Redacted rollback transcript, restore proof, data-integrity check, owner approval, timing notes, and cold-site DR RTO/RPO measurements from a production-like environment. | **REQUIRES_ENVIRONMENT / ACCEPTED_RISK_PENDING** | Blocks full Core GA unless waived. |
...
| P0-002 | Rollback and restore drill requires launch-environment execution. | SRE owner | **Medium** | Redacted rollback transcript, restore proof, data-integrity check, owner approval, timing notes, and cold-site DR RTO/RPO measurements. | REQUIRES_ENVIRONMENT | Blocks launch if rollback or restore cannot be executed within the approved recovery target. |
...
| ID | Item | Owner | Required Evidence | Current Status | Decision Rule |
|---|---|---|---|---|---|
| P0-001 | Production-like E2E launch rehearsal | Test owner | All launch-scope P0 journeys must include live staging evidence with real login, live backing services, persisted state, logs, and release-candidate SHA. | **RE-TESTABLE — AUTH UNBLOCKED** | The legacy-auth Clerk hook boundary crash is fixed (`RootRedirect` split into legacy/Clerk components; new behavior tests pass). The `case-meridian-e2e-001` workspace seed is now deterministic. Remaining validation requires a configured staging environment with Playwright and live backing services. |
| P0-002 | Rollback and restore drill | SRE owner | Redacted rollback transcript, restore proof, data-integrity check, owner approval, timing notes, and cold-site DR RTO/RPO measurements. | **RE-TESTABLE — PROCEDURE DOCUMENTED** | `docs/runbooks/deployment-rollout-and-rollback.md` now contains an explicit "Immutable Image / Dependency Rollback Requirement" section. `docs/troubleshooting/runbooks/incident/cold-site-dr-sync-strategy.md` defines cold-site drill evidence. `signoff-evidence/p0-rollback-20260613.json` was refreshed and `scripts/ci/verify_release_rollback.py` passes 8/8. A full environment rollback and cold-site DR rehearsal remains required for closure. |
| P0-003 | Enterprise SSO/OIDC provider validation | Identity owner | Provider configuration evidence, successful login/logout, failed-login handling, group/role mapping, and redacted audit event. | **RE-TESTABLE — LOCAL SURROGATE COMMITTED** | A `keycloak` service is now defined in `docker-compose.live.yml` under the `sso` profile; it imports `infra/keycloak/fabric-realm.json` and exposes port 8080. Realm import validation passes and manifest secret hygiene is clean. Live endpoint validation requires a Docker-enabled environment; historical token/claim checks remain valid. Real enterprise IdP integration remains environment-dependent. |
...
**NO GO for Core GA** on candidate `rc-2026-06-13-116815f3` remains the canonical decision. However, the candidate is now **re-testable**: the runtime auth crash is resolved, the `case-meridian-e2e-001` seed is deterministic, a viable rollback doctrine is documented, and a local SSO/OIDC surrogate is committed to the live compose stack. The remaining P0/P1 closure items require a configured staging/production-like environment and attached evidence. See `docs/readiness/launch-decision-artifact.md` for the canonical decision package.
...
| **P0-002** | SRE owner | Full rollback/restore and cold-site DR rehearsal requires a production-like environment with immutable, commit-pinned images and measured RTO/RPO; static rollback verifier passes and doctrine is documented. | Countersigned via approved Phase 1 plan | 2026-06-16 |
