...
| **P0-003** | Enterprise SSO/OIDC provider validation | Identity owner | **Variable** — High if enterprise SSO is a contracted Core GA requirement; otherwise acceptable residual risk for post-GA. | Provider configuration evidence, successful login/logout, failed-login handling, group/role mapping, and redacted audit event against a real enterprise IdP. | **REQUIRES_ENVIRONMENT / ACCEPTED_RISK_PENDING** | Blocks full Core GA if enterprise SSO is in scope and evidence is missing; can be scoped out of Core GA per `docs/launch/sso-core-ga-scope-decision.md`. |
...
| P0-003 | Enterprise SSO/OIDC provider validation requires configured provider credentials and tenant mapping. | Identity owner | **Variable** — High if enterprise SSO is a contracted Core GA requirement; otherwise acceptable residual risk for post-GA. | Provider configuration evidence, successful login/logout, failed-login handling, group/role mapping, and redacted audit event. | REQUIRES_ENVIRONMENT | Blocks enterprise launch if identity validation is incomplete or fails closed incorrectly; scope decision recorded in `docs/launch/sso-core-ga-scope-decision.md`. |
