...
| RG-2026-05-18-02 | `make verify` canonical gate | Current local evidence: `make lint` passes for Layers 1-6; full `make verify` timed out before producing a complete result. | Release owner | OPEN | Blocks launch until `make verify` is green. |
...
| RG-2026-05-18-05 | Artifact schema validation (`validate_live_workflow_artifacts.py`) | Current validator result: `live-workflow-validation-summary.json` is missing from `artifacts/live-workflow-validation/`. The prior `artifactPresence.composeConfig` failure is superseded until a fresh live-workflow evidence bundle is generated. | QA/CI | OPEN | Blocks acceptance of live evidence bundle. |
