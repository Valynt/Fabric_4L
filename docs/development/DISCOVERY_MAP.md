...
| Agent workflow, prompt, or tool | `services/layer4-agents/`, `contracts/agent-registry/`, `services/layer4-agents/prompts/`, `services/layer4-agents/skills/` | Output schema, checkpoint/resume state, provider adapter boundary, frontend consumer, Layer 5/6 dependency, prompt content hash, operating contract (tools/memory/permissions/eval_target) | `pnpm test:agents` plus targeted Layer 4 pytest when needed; `make check-prompt-registry` for prompt-version and operating-contract drift | `make gate-agent`, `make evals` when prompt behavior changes |
