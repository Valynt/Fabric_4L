...
**Canonical public API base:** `/v1/` on the **unified API gateway** (`services/api`).

All browser and external client traffic enters through the gateway. The gateway is the **sole** public ingress.
...
`k8s/routing/nginx/ingress.yaml` `layer-apis` Ingress **must** route all `/v1/*` traffic to the `api-gateway` service (port 8000), not to individual layer services.

The `/layerN` paths are **deprecated** and will be removed after the gateway delegation is verified in all environments.
...
2. **Phase 1:** Update K8s ingress to route `/v1/*` → gateway; remove `/layerN` paths
