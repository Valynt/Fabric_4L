...
| ARCH-001 | `VERIFIED_OPEN` | PR #1008 extracted workspace routes from the Layer 4 analysis hotspot; PR #1009 added frontend characterization; PR #1012 reconciled the type-escape baseline; and PR #1014 removed the ASGI test-harness timeout. The merged Layer 4 analysis-route suite passes 13/13, but `ProspectPromptBuilder.tsx` remains a 1,259-line frontend hotspot and has not been split. | Preserve the passing characterization while splitting the frontend hotspot along tested state/component boundaries; run targeted frontend, Layer 4, contract, and type checks. | PRs #1005, #1008, #1009, #1012, and #1014 plus a remaining frontend modularization PR. |
...
| DOC-002 | `VERIFIED_CLOSED` | PR #1002 merged the corrected README setup command and its documentation contract; the merged-main command-map suite passes 29/29. | Compare README with the `setup` target and `BUILD_SYSTEM.md`; docs tests pass on merged main. | PR #1002. |
...
- ARCH-001: PR #1008 extracted Layer 4 workspace routes, PR #1009 added frontend characterization, PR #1012 reconciled the type-escape baseline, and PR #1014 fixed the ASGI dependency-override hang. On merged commit `89b3e0a88`, `test_analysis_routes.py` passed 13/13. The finding remains open because the characterized `ProspectPromptBuilder.tsx` hotspot is still 1,259 lines and has not been split.
- DOC-002: PR #1002 merged as `8935bb2ff`; the merged-main documentation command-map suite passed 29/29.
- SEC-002: external credential revocation remains unverifiable from repository state and stays `EXTERNAL_VERIFICATION_REQUIRED`.
