...
| DEC-FE-007 | Route/nav integration through existing canon: `navigationService` route state `studio-mission`, `NAV_SCHEMA` child under Value Studio, served by the existing `studio/:tabId` router entry under its existing `accountStdPolicy("studio.workspace")` policy — no separate route or policy is registered | One source of truth for paths; navigation appears in the sidebar and route inventory without new conventions | Closed |
| DEC-FE-008 | Decision chrome placement and degraded-state consistency (PR #1679 review remediation): the decision surface is injected into the single StudioShell-owned right rail via `useStudioDetailRail` (the page renders no second, page-local rail); while the projection is stale or offline, ALL mission mutations are disabled consistently (decision submissions per FE-RAIL-008/009, plus Pause/Resume and activity Undo); `GenerativeUIFallbackBoundary` resets when the projection identity it wraps changes (`resetKey`) | Single-chrome rule (DEC-FE-001); controls must never contradict the degraded-state banners; a transient render failure must not latch the fallback after the projection recovers | Closed |
