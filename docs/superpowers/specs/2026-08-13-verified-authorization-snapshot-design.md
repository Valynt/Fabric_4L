...
denied. Missing snapshots, malformed claims, tenant mismatches, and fetch
failures expose no grants. The snapshot role is informational: configured
Clerk organization roles are accepted because grants come from the verified
permission set, not from a frontend role allowlist.
...
typed API client and validates the response before exposing it. Layer 4 builds
that response from authenticated `RequestContext` grants and rejects a tenant
slug outside the authenticated tenant. A verified
...
incorrect shapes and missing roles rather than constructing compatibility
grants. It does not maintain a second allowlist of Clerk roles.
...
Tests prove that unknown and absent display roles do not normalize to
`standard`; configured Clerk roles are accepted without inferring grants;
malformed snapshot claims, missing snapshots, tenant-mismatched snapshots,
expired snapshots, and snapshot-fetch failures expose no grants. Layer 4 tests
prove the endpoint is registered, emits only authenticated-context grants, and
rejects tenant mismatch. Hook tests distinguish snapshot status from permission
