...
- Create: `apps/web/src/hooks/useAuthorizationSnapshot.test.ts`
- Modify: `apps/web/src/hooks/useUserPermissions.ts`
- Create: `apps/web/src/hooks/useUserPermissions.test.ts`
- Create: `services/layer4-agents/src/layer4_agents/api/routes/authorization_snapshot.py`
- Modify: `services/layer4-agents/src/layer4_agents/api/routers.py`
- Create: `services/layer4-agents/tests/test_authorization_snapshot_routes.py`
- Modify: `contracts/openapi/layer4-agents.json`

**Interfaces:**

- Produces: `GET /v1/authz/snapshot`, `AuthorizationSnapshotState`, `VerifiedAuthorizationSnapshot`, `AuthorizationDecision`, `useAuthorizationSnapshot(tenantSlug)`, and `useUserPermissions(requiredPermissions, tenantSlug)`.
- Consumes: typed `apiGet` and Layer 4's authenticated `RequestContext`.

- [ ] Write parser and hook tests for loading, verified, absent/malformed claims, configured Clerk roles, tenant mismatch, expiration/refresh, and fetch failure.
...
- [ ] Implement the Layer 4 endpoint so permissions, entitlements, and account scopes come only from the authenticated context and a mismatched tenant slug returns 403; regenerate the OpenAPI contract.
