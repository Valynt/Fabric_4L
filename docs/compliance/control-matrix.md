...
- **Update trigger:** Any PR that changes security/governance-sensitive files must also evaluate (and, when needed, update) this matrix. Updated for CI-baseline repair (PR #1585) covering CI-control-plane structural contract tests in `pr-checks.yml`, runtime contract readiness hardening (L1–L6 health + strict contract env), and recurring CI-failure backlog reporting in `pr-backlog-health.yml`. Updated for Event Catalog validation gate (PR #1588) enforcing canonical event ownership, schema linkage, consumer version declarations, and deprecation policy in `pr-checks.yml`. Updated for R2 versioned shared-boundary surfaces (PR #1594) adding the bounded-change policy: each shared boundary (e.g., `shared.identity`) declares a `SURFACE_VERSION` marker and `scripts/ci/check_shared_boundary_surfaces.py` (wired into `pr-checks.yml` preflight) enforces that contract changes require a coordinated version bump and a regenerated `config/ci/shared_surface_contract.json`. Further updated to document runtime-contract secret validation hardening in `pr-checks.yml` (see "PR / CI Enforcement"). Updated for billing domain de-duplication (Brooks-Lint R3): `services/layer7-billing/` was removed and Layer 4 exclusively owns the billing domain; billing API routes remain mounted under `/v1/billing` in L4 with equivalent controller/webhook/tenant-isolation contracts, and `pr-checks.yml` test-skip registrations (`config/ci/test_skip_register.yaml`) and monitoring/k8s manifests were reconciled to the consolidated service. Updated for Observability Platform v1 (PR #1653): shared fail-closed OTEL client, audit `details.trace_id`/`span_id` correlation without a schema migration, and `monitoring/slo/slos.contract.json` drift-checked against recording/alert rules. Updated for ADR Registry + CI (PR #1655): `scripts/ci/check_adr.py` (wired via the `check-adr` Makefile target into `pr-checks.yml`) enforces fail-closed ADR registry completeness, numbering, related-code link existence, declared content assertions, and index-table drift across `docs/explanations/adr/` and `docs/decisions/`.
...
| --- | --- | --- | --- | --- | --- |
| GDPR Art. 5(1)(f), Art. 32 / CCPA §1798.150 / SOC 2 CC6.1, CC6.6 | Protect confidentiality and integrity of personal data in transit and at service boundaries. | Unified auth + tenant-resolution middleware in `shared/identity/middleware.py` enforces JWT/API key validation and resolves tenant context before protected requests proceed; API gateway ingress enforcement (`k8s/routing/*`) and layer network policies (`k8s/base/network-policies/*`) ensure all ingress routes through the authenticated gateway; `X-Tenant-ID` is mandatory for API access in docs. | Security controls are exercised in CI via PR and security workflows (`pr-checks.yml`, `security-gates.yml`) for lint/tests/scans, routing verification (`scripts/ci/k8s_routing_check.py`), and dependency checks. | Security Engineering | Monthly + per material auth/ingress change |
...
| GDPR Art. 30, Art. 5(2), Art. 33 / CCPA accountability / SOC 2 CC7.2, CC7.3 | Maintain immutable audit trail for security/compliance-relevant actions. | `003_add_audit_events.py` creates `audit_events` and DB trigger to block UPDATE/DELETE; shared audit models/emitter in `shared/audit/models.py` and `shared/audit/emitter.py` standardize action taxonomy and structured logging. Emitter stamps low-cardinality `details.trace_id`/`span_id` from `shared/observability` context (no audit-table migration; does not overwrite caller-supplied ids). SLO availability/latency/isolation targets are encoded in `monitoring/slo/slos.contract.json` and drift-checked against Prometheus recording/alert rules. | Incident/ops runbooks under `docs/runbooks/` + runbook index (`docs/runbooks/README.md`) provide investigation and response playbooks that rely on trace/audit data. | Security Engineering + SRE | Monthly + after incident |
...
| GDPR Art. 20 / CCPA portability expectation / SOC 2 CC2.3 | DSAR **Export/Portability**: provide user-consumable export of subject-associated artifacts where supported. | Export paths exist in Layer 4 analysis/tooling (`src/api/routes/analysis.py` export endpoint, `src/tools/document_export.py`). The deprecated Layer 3 export proxy (`/v1/documents/export`) was removed; business case exports flow directly from the frontend to Layer 4. | Export SLA: deliver first export package or status update within 7 days of validated request; failures routed via workflow/incident runbooks (`docs/runbooks/workflow-stalled.md`, `high-error-rate.md`). | Product Engineering + Privacy Operations | Monthly |
...
- ADR registry gate: `pr-checks.yml` runs `scripts/ci/check_adr.py` (wired via the `check-adr` Makefile target) as a fail-closed architecture-decision conformance gate: every discovered ADR in `docs/explanations/adr/` and `docs/decisions/` must be registered in `docs/decisions/adr-registry.yaml` (and vice versa), required `related` code paths must exist on disk, declared `must_contain`/`must_not_contain` content assertions must hold, numbering policy and header/sequence rules are enforced per corpus, and both ADR README index tables must match discovered files. Registry or index drift blocks merge before decisions silently rot.
