...
if PromCounter is not None:
    ...
    def _get_or_create_counter(
        ...
        existing = REGISTRY._names_to_collectors.get(
            name
        ) or REGISTRY._names_to_collectors.get(collector_name)
...
def record_deprecated_legacy_field_usage(
    field: str, *, tenant_id: str, app_client: str
) -> None:
...
def get_compat_metrics_snapshot() -> dict[str, dict[str, int]]:
    return {
        "route_hits": {
            "|".join(key): value for key, value in _DEPRECATED_ROUTE_HITS.items()
        },
        "legacy_field_hits": {
            "|".join(key): value for key, value in _DEPRECATED_LEGACY_FIELD_HITS.items()
        },
    }
