...
class TestL2CeleryDispatchRuntime:
    ...
    @patch("layer1_ingestion.shared.config.settings")
    def test_extract_and_ingest_flag_in_kwargs(self, mock_settings):
        """Celery dispatch should include use_extract_and_ingest flag."""
        mock_settings.use_celery_for_l2 = True
        mock_settings.layer2_celery_broker_url = "redis://redis:6379/0"
        ...
        call_args = mock_celery_instance.send_task.call_args
        assert call_args[1]["kwargs"]["use_extract_and_ingest"] is True

    @patch("layer1_ingestion.shared.config.settings")
    def test_source_version_id_in_extraction_payload(self, mock_settings):
        """Extraction payload should include source_version_id for L3 tracking."""
        mock_settings.use_celery_for_l2 = True
        mock_settings.layer2_celery_broker_url = "redis://redis:6379/0"
        ...
        call_args = mock_celery_instance.send_task.call_args
        args_list = call_args[1]["args"]
        payload_arg = args_list[3]
        assert payload_arg["source_version_id"] == source_version_id
