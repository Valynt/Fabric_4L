...
def _create_sync_redis_client():
    """Create a synchronous Redis client for the kill-switch fallback.

    Returns None if Redis is unavailable so the middleware falls back to
    UNKNOWN and fails closed, rather than crashing during import.
    """
...
def configure_middleware(app: FastAPI) -> None:
    app.add_middleware(
        ...
        redis_client=_create_sync_redis_client(),
