...
def test_authorization_snapshot_route_is_registered() -> None:
    ...
    assert "/v1/authz/snapshot" in app.openapi()["paths"]


@pytest.mark.asyncio
async def test_authenticated_tenant_receives_verified_authorization_snapshot(
    monkeypatch,
) -> None:
    ...
    response = await get_authorization_snapshot(tenant_slug="acme", db=object(), ctx=context)
...
async def test_snapshot_rejects_slug_outside_authenticated_tenant(monkeypatch) -> None:
    ...
    with pytest.raises(HTTPException) as exc_info:
        await get_authorization_snapshot(tenant_slug="other", db=object(), ctx=context)
