...
def test_share_link_store_unavailable_returns_503():
    ...
    try:
        db.accounts.insert(
            "acc-store-fail",
            Account(id="acc-store-fail", tenant_id=TENANT_ALPHA, name="x", industry="y"),
        )
        with TestClient(app) as client:
            res = client.post("/v1/accounts/acc-store-fail/share", headers=auth_headers(TENANT_ALPHA))
...
def test_impersonation_store_unavailable_fails_closed():
    ...
    try:
        ...
        token = create_access_token(subject=admin.id, tenant_id=TENANT_ALPHA)
        with TestClient(app) as client:
            res = client.post(
                "/v1/auth/impersonation/start",
                json={"target_user_id": target.id, "reason": "support"},
                headers={"Authorization": f"Bearer {token}", "X-Tenant-ID": TENANT_ALPHA},
            )
            assert res.status_code == 503
    finally:
