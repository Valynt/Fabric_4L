...
def test_same_tenant_wrong_account_cannot_read_signals() -> None:
    allowed_headers = auth_headers(tenant_id=TENANT_ALPHA, extra_claims={"account_ids": ["acc-allowed"]})
...
def test_same_tenant_wrong_account_cannot_traverse_stakeholders() -> None:
    ...
    denied = client.get('/v1/accounts/acc-forbidden/stakeholders', headers=allowed_headers)
    assert denied.status_code == 403
