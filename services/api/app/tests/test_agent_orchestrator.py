...
class TestAgentOrchestratorDelegation:
    ...
    def test_create_run_delegates_and_projects(self) -> None:
        ...
        run = orchestrator.create_run(
            tenant_id="t1", workflow_type="roi_calculator", account_id="acc-1"
        )
        assert run.id == "wf-1"
    ...
    def test_get_run_refreshes_from_layer4(self) -> None:
        orchestrator = self._orchestrator()
        run = orchestrator.create_run(tenant_id="t1", workflow_type="roi_calculator")
        orchestrator.layer4_client.workflows[run.id]["status"] = "running"
        refreshed = orchestrator.get_run(run.id, tenant_id="t1")
        assert refreshed is not None
        assert refreshed.status == "running"

    def test_get_run_missing_projection_returns_none(self) -> None:
        orchestrator = self._orchestrator()
        assert orchestrator.get_run("wf-missing", tenant_id="t1") is None

    def test_pause_resume_cancel_delegate(self) -> None:
        orchestrator = self._orchestrator()
        run = orchestrator.create_run(tenant_id="t1", workflow_type="roi_calculator")

        paused = orchestrator.pause_run(run.id, tenant_id="t1", user_id="u1")
        assert paused.status == "paused"

        resumed = orchestrator.resume_run(run.id, tenant_id="t1", user_id="u1")
        assert resumed.status == "running"

        cancelled = orchestrator.cancel_run(run.id, tenant_id="t1")
