...
# ===========================================================================
# Log-error sanitization (URL userinfo scrubbing)
# ===========================================================================


class TestSanitizeLogError:
    def test_scrubs_amqp_userinfo(self):
        from ..helpers import sanitize_log_error

        msg = 'Connection refused: amqp://broker:secretpass@rabbitmq:5672/vhost'
        out = sanitize_log_error(msg)
        assert 'secretpass' not in out
        assert 'broker' not in out
        assert 'amqp://***@rabbitmq:5672/vhost' in out

    def test_scrubs_redis_userinfo(self):
        from ..helpers import sanitize_log_error

        msg = 'Connection refused: redis://:hunter2@localhost:6379/0'
        out = sanitize_log_error(msg)
        assert 'hunter2' not in out
        assert 'redis://***@localhost:6379/0' in out

    def test_scrubs_userinfo_without_password(self):
        from ..helpers import sanitize_log_error

        msg = 'amqps://svc-prod@rabbitmq.example.com/ timed out'
        out = sanitize_log_error(msg)
        assert 'svc-prod' not in out
        assert 'amqps://***@rabbitmq.example.com/' in out

    def test_plain_message_unchanged_except_normal(self):
        from ..helpers import sanitize_log_error

        msg = 'dispatch failed for target acct-123: connection reset'
        assert sanitize_log_error(msg) == msg

    def test_bearer_token_still_redacted(self):
        from ..helpers import sanitize_log_error

        assert sanitize_log_error('Authorization: Bearer abc123') == '[REDACTED: contains bearer]'

    def test_exception_uses_repr_and_still_scrubs(self):
        from ..helpers import sanitize_log_error

        exc = ConnectionError('amqp://user:pass@mq:5672/ down')
        out = sanitize_log_error(exc)
        assert 'user:pass' not in out
        assert 'amqp://***@mq:5672/' in out
