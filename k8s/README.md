...
- **Retention policy**: Trace retention is explicitly enabled through Jaeger ILM flags (`ES_USE_ILM=true`, `ES_ILM_POLICY_NAME=jaeger-hot-7d`) and governed by the OpenSearch/Elasticsearch Index State Management policy for `jaeger-span*`. Default platform target is **7 days hot retention** unless SRE sets an environment-specific override.
...
- **Staging trace continuity validation**: After deploying staging, create a trace via any OTLP source, capture its trace ID, restart the Jaeger pod (`kubectl -n monitoring rollout restart deploy/jaeger`), then verify the same trace ID remains queryable in Jaeger UI/API. This confirms durable backend persistence across pod restarts.
