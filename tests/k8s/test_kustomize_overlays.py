...
def test_dev_nginx_replaces_ingress_namespace_placeholder() -> None:
    overlay = Path("k8s/deployments/dev-nginx/kustomization.yaml")
    host_config = Path("k8s/deployments/dev-nginx/hostname-config.yaml")

    overlay_data = yaml.safe_load(overlay.read_text(encoding="utf-8"))
    host_data = yaml.safe_load(host_config.read_text(encoding="utf-8"))

    assert host_data["data"]["ingressNamespace"] == "ingress-nginx"
    replacement_sources = [replacement["source"].get("fieldPath") for replacement in overlay_data["replacements"]]
    assert "data.ingressNamespace" in replacement_sources

    ingress_replacement = next(
        replacement
        for replacement in overlay_data["replacements"]
        if replacement["source"].get("fieldPath") == "data.ingressNamespace"
    )
    targets = {target["select"]["name"]: target["fieldPaths"] for target in ingress_replacement["targets"]}
    placeholder_field = "spec.ingress.0.from.0.namespaceSelector.matchLabels.[kubernetes.io/metadata.name]"
    assert placeholder_field in targets["frontend-policy"]
    assert placeholder_field in targets["oauth2-proxy"]


def test_env_overlays_replace_frontend_ingress_namespace_placeholder() -> None:
    placeholder_field = "spec.ingress.0.from.0.namespaceSelector.matchLabels.[kubernetes.io/metadata.name]"

    for overlay in (
        Path("k8s/envs/dev/kustomization.yaml"),
        Path("k8s/envs/staging/kustomization.yaml"),
        Path("k8s/envs/prod/kustomization.yaml"),
    ):
        overlay_data = yaml.safe_load(overlay.read_text(encoding="utf-8"))
        ...
        assert ingress_replacement is not None, f"{overlay} must replace ingress namespace"
        assert "../../routing/nginx/routing-config.yaml" not in overlay_data.get("resources", [])

        generators = overlay_data.get("configMapGenerator", [])
        routing_generator = next(
            (generator for generator in generators if generator.get("name") == "routing-config"),
            None,
        )
        assert routing_generator is not None, f"{overlay} must define routing-config locally"
        assert "ingressNamespace=ingress-nginx" in routing_generator.get("literals", [])
        assert overlay_data.get("generatorOptions", {}).get("disableNameSuffixHash") is True

        targets = {
            target["select"]["name"]: target["fieldPaths"]
            for target in ingress_replacement["targets"]
        }
        assert placeholder_field in targets["frontend-policy"]


def test_nginx_deployment_overlays_do_not_reimport_routing_config() -> None:
    for overlay in (
        Path("k8s/deployments/dev-nginx/kustomization.yaml"),
        Path("k8s/deployments/prod-nginx/kustomization.yaml"),
    ):
        overlay_data = yaml.safe_load(overlay.read_text(encoding="utf-8"))
        resources = overlay_data.get("resources", [])

        assert "../../routing/nginx" not in resources
        assert "../../routing/nginx/routing-config.yaml" not in resources
        assert "../../routing/nginx/clusterissuer.yaml" in resources
        assert "../../routing/nginx/ingress.yaml" in resources
        assert "../../routing/nginx/oauth2-proxy.yaml" in resources


def test_kyverno_keyless_attestors_use_v1_11_rekor_shape() -> None:
    for policy_path in (
        Path("k8s/policy/kyverno-slsa-provenance.yaml"),
        Path("k8s/policy/kyverno-verify-signatures.yaml"),
    ):
        text = policy_path.read_text(encoding="utf-8")
        assert "rekorURL" not in text
        assert "rekor:\n                          url: https://rekor.sigstore.dev" in text
