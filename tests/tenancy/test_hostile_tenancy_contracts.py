...
pytestmark = [pytest.mark.security, pytest.mark.tenant_boundary, pytest.mark.production_readiness]
...
def test_signed_url_replay_and_expiration_denial(harness: HostileTenancyHarness):
    ...
    data = harness.access_signed_url("sig-alpha-valid", requesting_tenant_id=TENANT_ALPHA_ID)
    ...
    with pytest.raises(PermissionError, match="replay denied"):
        harness.access_signed_url("sig-alpha-valid", requesting_tenant_id=TENANT_ALPHA_ID)

    # 3. Expired signed URL must fail closed
    with pytest.raises(PermissionError, match="expired"):
        harness.access_signed_url("sig-alpha-expired", requesting_tenant_id=TENANT_ALPHA_ID)

    # 4. Cross-tenant signed URL access denial: Tenant Beta cannot use Tenant Alpha's signature
    now = datetime.now(timezone.utc)
    sig_alpha_fresh = harness.create_signed_url("sig-alpha-fresh", TENANT_ALPHA_ID, "exports/2026/alpha-q3.csv", now + timedelta(minutes=10))
    with pytest.raises(PermissionError, match="Cross-tenant signed URL access denied"):
        harness.access_signed_url(sig_alpha_fresh.signature, requesting_tenant_id=TENANT_BETA_ID)
...
def test_object_storage_key_and_prefix_isolation(harness: HostileTenancyHarness):
    ...
    res_beta = harness.assert_foreign_resource_exists(TENANT_BETA_ID, "document", "doc-beta-002")
    ...
    with pytest.raises(PermissionError, match="Cross-tenant object access denied"):
        harness.get_object(requesting_tenant_id=TENANT_ALPHA_ID, target_tenant_id=TENANT_BETA_ID, key=beta_key)

    # Tenant Alpha accessing their own object succeeds
    alpha_data = harness.get_object(requesting_tenant_id=TENANT_ALPHA_ID, target_tenant_id=TENANT_ALPHA_ID, key="exports/2026/alpha-q3.csv")
...
def test_graph_and_vector_retrieval_isolation(harness: HostileTenancyHarness):
    ...
    harness.assert_foreign_resource_exists(TENANT_BETA_ID, "graph_entity", "node-beta-202")

    # 1. Graph Entity lookup: Tenant Alpha cannot read Tenant Beta's node
    with pytest.raises(PermissionError, match="Cross-tenant graph entity access denied"):
        harness.get_graph_entity(requesting_tenant_id=TENANT_ALPHA_ID, entity_id="node-beta-202")

    # Tenant Alpha can read own node
    alpha_node = harness.get_graph_entity(requesting_tenant_id=TENANT_ALPHA_ID, entity_id="node-alpha-101")
    assert alpha_node["name"] == "Alpha Corp Entity"

    # 2. Vector search isolation: Tenant Alpha query vector returns only Alpha vectors
    results = harness.query_vectors(requesting_tenant_id=TENANT_ALPHA_ID, embedding=[0.1, 0.2, 0.3])
...
def test_ai_memory_trace_and_cache_isolation(harness: HostileTenancyHarness):
    ...
    with pytest.raises(PermissionError, match="Cross-tenant AI session access denied"):
        harness.get_ai_session_memory(requesting_tenant_id=TENANT_ALPHA_ID, session_id="sess-beta-1")

    # Tenant Alpha can read own session
    alpha_history = harness.get_ai_session_memory(requesting_tenant_id=TENANT_ALPHA_ID, session_id="sess-alpha-1")
    ...
    with pytest.raises(PermissionError, match="Cross-tenant AI trace access denied"):
        harness.get_ai_trace(requesting_tenant_id=TENANT_ALPHA_ID, trace_id="trace-beta-002")
...
def test_prior_session_and_tenant_switch_rejection(harness: HostileTenancyHarness):
    ...
    with pytest.raises(PermissionError, match="Cross-tenant AI session access denied"):
        harness.get_ai_session_memory(requesting_tenant_id=active_tenant, session_id=session_alpha)
...
def test_support_impersonation_scope_and_expiration(harness: HostileTenancyHarness):
    ...
    res = harness.execute_with_impersonation(grant.grant_id, action_tenant_id=TENANT_BETA_ID, action_scope="read:billing")
    ...
    with pytest.raises(PermissionError, match="Impersonation scope mismatch"):
        harness.execute_with_impersonation(grant.grant_id, action_tenant_id=TENANT_BETA_ID, action_scope="write:billing")
    ...
    with pytest.raises(PermissionError, match="Impersonation scope violation"):
        harness.execute_with_impersonation(grant.grant_id, action_tenant_id=TENANT_ALPHA_ID, action_scope="read:billing")
    ...
    with pytest.raises(PermissionError, match="expired"):
        harness.execute_with_impersonation(expired_grant.grant_id, action_tenant_id=TENANT_BETA_ID, action_scope="read:billing")
