...
def test_to_layer5_claim_type_rejects_falsy() -> None:
    """Disclosed behavior change (F-4): falsy claim_type fails fast.

    The pre-canonical business_case mapping silently defaulted falsy values
    to ``metric`` -> ``value_driver_metric``. The canonical function raises
    ValueError instead, so a missing claim type can no longer be promoted to
    Layer 5 under a fabricated category.
    """
    claim_types = _l4_module()
    for falsy in (None, "", 0):
        with pytest.raises(ValueError):
            claim_types.to_layer5_claim_type(falsy)
    with pytest.raises(ValueError):
        claim_types.require_canonical_claim_type("")
