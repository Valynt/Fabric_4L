...
@pytest.mark.unit
def test_layer1_observability_endpoints_contract(
    ...
    assert_probe_response_shape(
        ...
        expected_json_keys={"status", "service"},
    )
    assert_probe_response_shape(
        client,
        path="/ready",
...
@pytest.mark.unit
def test_layer3_observability_endpoints_contract(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.unit
def test_layer5_observability_endpoints_contract(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        l5_main, "_check_schema_migration_alignment", _mock_schema_alignment
    )
...
@pytest.mark.unit
def test_layer6_observability_endpoints_contract(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _force_test_startup_environment(monkeypatch)
...
@pytest.mark.unit
def test_api_gateway_observability_endpoints_contract(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import httpx
    import respx
    ...
    monkeypatch.setattr(
        "app.services.distributed_store.get_distributed_store",
        lambda: InMemoryDistributedStore(),
    )
    ...
    def mock_layer4_ready(url, *args, **kwargs):
        if url == layer4_url:
            return httpx.Response(
                200, json={"status": "ready", "service": "layer4-agents"}
            )
