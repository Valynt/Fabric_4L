...
def test_container_jobs_depend_on_ci_tools_preflight() -> None:
    ...
    users = {
        ...
        and job["container"].get("image") == "${{ env.CI_TOOLS_IMAGE }}"
    }
...
def test_preflight_pulls_and_asserts_digest_and_all_versions() -> None:
    ...
    for command in (
        "grype version", "syft version", "cosign version", "pip-audit --version",
        "pip-licenses --version", "python --version", "node --version", "pnpm --version",
    ):
        assert command in run_scripts


def test_publisher_uses_pinned_toolchain_and_trusted_identity() -> None:
    dockerfile = (ROOT / "tools" / "ci" / "security-suite" / "Dockerfile").read_text(encoding="utf-8")
    for pin in (
        "python:3.12.10-slim-bookworm@sha256:",
        "GRYPE_VERSION=0.104.1", "SYFT_VERSION=1.30.0", "COSIGN_VERSION=2.5.3",
        "PIP_AUDIT_VERSION=2.9.0", "PIP_LICENSES_VERSION=5.0.0",
        "NODE_VERSION=22.17.0", "PNPM_VERSION=10.18.1",
    ):
    ...
    publish_step = next(
        step for step in publisher["jobs"]["publish"]["steps"]
