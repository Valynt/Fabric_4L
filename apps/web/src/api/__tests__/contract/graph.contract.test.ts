...
describe('Contract: graph auth failures', () => {
  it('401 matches ApiError shape', () => {
    assertSchema(
      ApiErrorSchema,
      { error: { message: 'Authentication required', code: 'AUTHENTICATION_ERROR', request_id: 'trace-graph-401' } },
      'ApiError (401)'
    );
  });
  ...
  it('403 cross-tenant entity access matches ApiError shape', () => {
    const err = assertSchema(
      ApiErrorSchema,
      { error: { message: 'Entity belongs to a different tenant', code: 'AUTHORIZATION_ERROR', request_id: 'trace-graph-403' } },
  ...
  it('entity not found 404 matches ApiError shape', () => {
    const err = assertSchema(
      ApiErrorSchema,
      { error: { message: 'Entity not found', code: 'NOT_FOUND', request_id: 'trace-graph-404' } },
      'ApiError (404)'
    );
