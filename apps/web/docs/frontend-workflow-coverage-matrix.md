...
| Step-by-step workflow inventory | `docs/frontend-user-workflows.md` lists each J0-J5, P0, and P1 user workflow path; this prose inventory is source-derived and must stay aligned to the executable contracts below. |
| Executable workflow contracts | `docs/frontend-workflow-contracts.json` defines actions, UI proof, backend proof, security invariants, audit invariants, failure cases, executable evidence files, and per-plane evidence ownership for UI/backend/security/audit proof on each P0/P1 workflow; `pnpm run test:workflow-contracts` enforces the contract. |
...
| Release evidence supplement | `pnpm run verify:frontend:release` requires `FRONTEND_RELEASE_EVIDENCE` to point at a redacted JSON file shaped like `docs/frontend-release-evidence.template.json`; `pnpm run test:frontend-release-evidence` fails closed unless the evidence records exact commit SHA, image digest, production-build status, live P0 browser journeys against real internal infra, production-like infrastructure components, no unexpected browser errors, no HTTP 5xx, no unhandled mock requests, no failed background jobs, authorization-matrix pass counts and actors, API and retrieval tenant-isolation results, approval/export/audit results, named live-provider smoke, named post-deploy synthetics, trace retention, and canonical release evidence packet status. |
...
| P1-SEARCH-SECURITY | Search and retrieval with tenant and role constraints | `/command-center`, `/context/*`, `/governance/evidence`, `/governance/traces` | `e2e/journeys/j18-search-retrieval.spec.ts`; `e2e/security/tenant-isolation-validation.spec.ts`; `e2e/security/tenant-isolation-deep.spec.ts`; `tests/security/test_hostile_tenant_e2e_matrix.py` | Cross-tenant leakage, result-to-context routing drift, role-filter bypass. |
