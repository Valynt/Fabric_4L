...
9.  **Incomplete PostgreSQL backup strategy** — While a backup CronJob exists, it lacks documented validation for offsite storage and recovery.
...
6.  Validate the PostgreSQL pg_dump CronJob against offsite storage and document the recovery runbook.
...
### PROD-P0-005: Incomplete PostgreSQL Backup Strategy
...
- **Tickets**: TICKET-SEC-004 (Auth bypass fix), PROD-P2-003 (Cleanup).

## Phase 1: Security & Tenant Isolation (Weeks 2-4)
- **Goal**: Close all P0 security gaps.
- **Tickets**: PROD-P0-002 (L2 Auth), PROD-P0-003 (L1 SSRF), PROD-P1-001 (S2S Auth), TICKET-SEC-004 (Auth bypass fix), TICKET-SEC-005 (Neo4j encryption).
...
- **Tickets**: PROD-P0-004 (FE Coverage), TICKET-SEC-006 (Demo data), TICKET-FE-001 (Entitlements).
