...
class FailureClassifier:
    ...
    def _result(category: FailureCategory, score: int, snippets: list[str], text: str) -> ClassificationResult:
        ...
        if category.key not in GOVERNANCE_CATEGORY_KEYS:
            raise ValueError(f"Primary category is not in governance taxonomy: {category.key}")
    ...
    def markdown(results: list[ClassificationResult]) -> str:
        lines = [
            "| Category | Secondary | Severity | Auto-fixable | Blocks GA | Strategy | Summary |",
            "|---|---|---|---:|---:|---|---|",
        ]
        for result in results:
            secondary = result.secondary_category_key or "-"
            lines.append(
                f"| {result.category_key} | {secondary} | {result.severity} | {str(result.auto_fixable).lower()} | "
