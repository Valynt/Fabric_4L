...
_PATTERNS: tuple[SignaturePattern, ...] = (
    ...
    SignaturePattern(
        "line_column",
        re.compile(
            r"(?P<path><(?:workspace_path|temp_path)>):(?P<line>\d+):(?P<col>\d+)"
        ),
        r"\g<path>:<line>:<col>",
    ),
    SignaturePattern(
        "line_column",
        re.compile(r"(?P<path><(?:workspace_path|temp_path)>):(?P<line>\d+)"),
        r"\g<path>:<line>",
    ),
    SignaturePattern(
        "line_column",
    ...
    SignaturePattern(
        "line_column",
        re.compile(
            r"(?P<file>[^\s:'\")\]]+\.(?:py|pyi|ts|tsx|js|jsx|mjs|cjs|rs|go|java|kt|rb|php|cs|cpp|c|h|hpp|yaml|yml|json|toml|md)):(?P<line>\d+)"
        ),
        r"\g<file>:<line>",
    ),
    SignaturePattern(
