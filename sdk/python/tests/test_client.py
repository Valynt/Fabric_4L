...
@pytest.fixture
def mock_client() -> Iterator[ValueFabricClient]:
...
class TestTenants:
    def test_list_tenants(self, mock_client: ValueFabricClient) -> None:
        tenants = mock_client.list_tenants()
        assert len(tenants) == 1
        assert tenants[0].name == "Acme"
        assert isinstance(tenants[0], Tenant)

    def test_get_tenant(self, mock_client: ValueFabricClient) -> None:
...
class TestUsers:
    def test_list_users(self, mock_client: ValueFabricClient) -> None:
        users = mock_client.list_users()
        assert len(users) == 1
        assert users[0].email == "alice@example.com"
        assert isinstance(users[0], User)

    def test_invite_user(self, mock_client: ValueFabricClient) -> None:
...
class TestApiKeys:
    def test_list_api_keys(self, mock_client: ValueFabricClient) -> None:
        keys = mock_client.list_api_keys()
        assert len(keys) == 1
        assert keys[0].name == "test-key"
        assert isinstance(keys[0], APIKey)

    def test_create_api_key(self, mock_client: ValueFabricClient) -> None:
...
class TestWorkflows:
    def test_list_workflows(self, mock_client: ValueFabricClient) -> None:
        workflows = mock_client.list_workflow_types()
        assert len(workflows) == 1
        assert workflows[0].type == "roi_calculator"
        assert isinstance(workflows[0], WorkflowTypeInfo)

    def test_list_active_workflows(self, mock_client: ValueFabricClient) -> None:
        workflows = mock_client.list_active_workflows()
        assert len(workflows.items) == 1
        assert workflows.items[0].id == "wf-1"
        assert isinstance(workflows, WorkflowListResponse)

    def test_execute_workflow(self, mock_client: ValueFabricClient) -> None:
        result = mock_client.execute_workflow("roi_calculator")
        assert result.workflow_instance_id == "wf-2"
        assert isinstance(result, WorkflowCreateResponse)

    def test_get_workflow(self, mock_client: ValueFabricClient) -> None:
        wf = mock_client.get_workflow("wf-1")
        assert wf.id == "wf-1"
        assert isinstance(wf, WorkflowStatus)


class TestModels:
    def test_list_models(self, mock_client: ValueFabricClient) -> None:
        models = mock_client.list_models()
        assert len(models) == 1
        assert models[0].model_name == "gpt-4"
        assert isinstance(models[0], ModelVersion)

    def test_promote_model(self, mock_client: ValueFabricClient) -> None:
...
class TestFeatureFlags:
    def test_list_feature_flags(self, mock_client: ValueFabricClient) -> None:
        flags = mock_client.list_feature_flags()
        assert len(flags) == 1
        assert flags[0].flag_key == "new_ui"
        assert isinstance(flags[0], FeatureFlag)

    def test_set_feature_flag(self, mock_client: ValueFabricClient) -> None:
...
class TestHealth:
    def test_health(self, mock_client: ValueFabricClient) -> None:
...
class TestAsyncClient:
    async def test_alist_tenants(self) -> None:
        transport = _mock_transport(
            {
                ("GET", "/v1/tenants"): (
                    200,
