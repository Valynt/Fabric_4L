...
class ValueFabricClient:
    ...
    @staticmethod
    def _safe_response_body(response: httpx.Response) -> dict[str, Any] | None:
        """Return a recursively redacted JSON object for exception context."""
        try:
            value = response.json()
        except ValueError:
            return None
        ...
        def redact(item: Any, key: str | None = None) -> Any:
            if key is not None and key.lower().replace("-", "_") in sensitive:
                return "[REDACTED]"
    ...
    def _handle_error(self, error: HTTPStatusError) -> NoReturn:
        """Convert an HTTP status failure to a safe SDK exception."""
        response = error.response
        status_code = response.status_code
        endpoint = error.request.url.path
        body = self._safe_response_body(response)
        message = f"API error ({status_code}): {error.request.method} {endpoint}"
        ...
        if status_code == 400:
            detail = (
                body.get("detail", "Request validation failed")
                if body
                else "Request validation failed"
            )
            raise ValidationError(
                str(detail), status_code=status_code, endpoint=endpoint, response_body=body
            ) from None
        if status_code == 404:
            raise NotFoundError(
        ...
        raise APIError(
            message, status_code=status_code, endpoint=endpoint, response_body=body
        ) from None
